# LLM Context（大语言模型上下文）

> **领域**: 人工智能 / 自然语言处理 | **复杂度**: 中高级 | **生成日期**: 2026-09-04

---

## 📖 一、个人解释（通俗理解）

**LLM Context（上下文）** 是大语言模型（如ChatGPT、Claude）在进行对话或生成文本时所参考的**所有背景信息的总和**。你可以把它想象成模型的"工作记忆"或"当前正在思考的内容"。

**🎯 一句话总结**:
Context = 模型在做决策时能"看到"的所有信息（系统指令 + 历史对话 + 当前问题 + 背景知识）

**💡 直观类比**:

想象你是一个**考试中的学生**：

- **System Prompt（系统提示）** = 考试规则和答题要求（写在试卷开头）
- **Conversation History（对话历史）** = 之前做过的题目和你的答案
- **Current Input（当前输入）** = 现在正在看的这道新题目
- **Knowledge Base（知识库）** = 你脑子里记住的公式、定理、知识点

**Context Window（上下文窗口）** = 你的**考试时间**或**草稿纸大小**——是有限的！

如果草稿纸太小（窗口短），你可能记不住前面做过什么，导致前后矛盾。
如果时间太长（虽然不太可能），你可能会被太多信息干扰，抓不住重点。

这就是 LLM Context 的核心挑战：**如何在有限的空间内，提供最相关、最有用的信息？**
...
**Context（上下文）** 在大语言模型中指的是：**输入到模型中进行推理的所有token序列**，包括系统提示、历史消息、用户输入、检索到的文档等。
